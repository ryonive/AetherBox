...
internal static class GameObjectHelper
{
	public static float GetTargetDistance(GameObject target)
	{
		...
		Vector2 position = new Vector2(target.Position.X, target.Position.Z);
		Vector2 selfPosition = new Vector2(Svc.ClientState.LocalPlayer.Position.X, Svc.ClientState.LocalPlayer.Position.Z);
	...
	{
		float dist = Svc.ClientState.LocalPlayer.Position.Y - target.Position.Y;
		if (dist < 0f)
		{
			dist *= -1f;
		}
