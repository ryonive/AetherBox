...
// AetherBox.Helpers.Faloop.FaloopSocketIOClient
using System;
...
public class FaloopSocketIOClient : IDisposable
{
	...
	private void HandleOnMessage(SocketIOResponse response)
	{
		...
		{
			FaloopEventPayload payload = response.GetValue(index).Deserialize<FaloopEventPayload>();
			if (payload != null && payload.Type == "mob" && payload.SubType == "report")
			{
				MobReportData data = payload.Data.Deserialize<MobReportData>();
				if (data != null)
				{
