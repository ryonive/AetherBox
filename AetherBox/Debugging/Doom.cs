    ...
    public static double GetSliderValue(float t, float x, int i)
    {
        ...
        if (i == 0)
        {
            ...
            for (int eIndex = 0; eIndex < Enemies.Length; eIndex++)
            {
                ...
                {
                    double a2;
                    a2 = Math.Atan2(e3[1] - Cam[1], e3[0] - Cam[0]);
        ...
        if ((i == 32 || i == 31) && pressed[32])
        {
            for (int index = 0; index < Enemies.Length; index++)
            {
                ...
                e2 = Enemies[index];
                double[] inter;
                inter = intersect2(Cam[0], Cam[1], end[0], end[1], e2[0], e2[1], 0.3);
        ...
        double nearestWall;
        nearestWall = (xs.Any() ? xs.Min() : 9999.0);
        IEnumerable<double> xs2;
        xs2 = from e in Enemies
              select intersect2(Cam[0], Cam[1], end[0], end[1], e[0], e[1], 0.3) into p
              where p != null && p.Length != 0
              select Math.Sqrt(Math.Pow(p[0] - Cam[0], 2.0) + Math.Pow(p[1] - Cam[1], 2.0));
        double nearestEnemy;
        nearestEnemy = (xs2.Any() ? xs2.Min() : 9999.0);
        ...
        static double[] intersect2(double x1, double y1, double x2, double y2, double cx, double cy, double r)
        {
            ...
            v2y = y1 - cy;
            double b;
            b = -2.0 * (v1x * v2x + v1y * v2y);
            double c;
            c = 2.0 * (v1x * v1x + v1y * v1y);
            double d2;
            d2 = Math.Sqrt(b * b - 2.0 * c * (v2x * v2x + v2y * v2y - r * r));
            if (double.IsNaN(d2))
            {
                return null;
            }
