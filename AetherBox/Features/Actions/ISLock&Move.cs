...
public class ISLockAndMove : Feature
{
    ...
    private unsafe void CheckToLockAndMove(ConditionFlag flag, bool value)
    {
        ...
        {
            List<GameObject> list = Svc.Objects.Where((GameObject x) => x.ObjectKind == ObjectKind.CardStand && x.IsTargetable).ToList();
            if (list.Count != 0)
            {
                GameObject gameObject = list.OrderBy((GameObject x) => Vector3.Distance(x.Position, Player.Object.Position)).FirstOrDefault();
